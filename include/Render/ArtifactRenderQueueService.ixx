...
public:
  ...
  void addRenderQueue();
  void
  addRenderQueueForComposition(const ArtifactCore::CompositionID &compositionId,
                               const QString &compositionName);

  // 複数形式でジョブを追加（After Effects 風）
  void
  addRenderQueueWithPreset(const ArtifactCore::CompositionID &compositionId,
                           const QString &compositionName,
                           const QString &presetId);
  void addMultipleRenderQueuesForComposition(
      const ArtifactCore::CompositionID &compositionId,
      const QString &compositionName, const QVector<QString> &presetIds);
  ...
  void removeRenderQueuesForComposition(
      const ArtifactCore::CompositionID &compositionId);
  bool hasRenderQueueForComposition(
      const ArtifactCore::CompositionID &compositionId) const;
  int renderQueueCountForComposition(
      const ArtifactCore::CompositionID &compositionId) const;
  ...
  void setJobOutputSettingsAt(int index, const QString &outputFormat,
                              const QString &codec, const QString &codecProfile,
                              int width, int height, double fps,
                              int bitrateKbps);
  void setJobOutputSettingsAt(int index, const QString &outputFormat,
  ...
  bool jobOverlayTransformAt(int index, float *offsetX, float *offsetY,
                             float *scale, float *rotationDeg) const;
  void setJobOverlayTransform(int index, float offsetX, float offsetY,
...
Q_DECLARE_METATYPE(Artifact::ArtifactRenderQueueService::FailedFrameInfo)
