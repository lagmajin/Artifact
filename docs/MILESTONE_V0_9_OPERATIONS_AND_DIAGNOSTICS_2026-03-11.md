...
- 進捗 (2026-03-11):
  ...
  - 次の情報をテキスト出力:
    ...
    - レイアウト復元結果（復元試行有無 / geometry/state 成否 / リセット有無 / 復元時刻）
    - 現在の Dock ウィジェット可視状態
    - メニューアクションの有効/無効状態スナップショット
...
- 完了条件:
  - 復旧手順・診断手順・再実行手順を1本化
- 進捗 (2026-03-11):
  - `docs/OPERATIONS_PLAYBOOK_V0_9_2026-03-11.md` を追加
  - 異常終了復旧/診断採取/RenderQueue失敗時/レイアウト破損時の運用手順を定義
