...
**Status:** Planned
**Goal:** Move clone-system features into a new `Clone Layer`, and remove the legacy `LayerTransform` rack from the normal Inspector flow.

## Current State

1. `Generator` - keep
2. `Geometry Transform` - keep, but clone-system elements may be separated
...
- Should clone-system elements be split out from `Geometry Transform`?
...
- clone-system behavior belongs to `Clone Layer`, not the old `LayerTransform` rack
...
1. Decide clone-system placement: `Geometry Transform` split vs `Generator` integration
...
- `Artifact/docs/MILESTONE_CLONER_GENERATOR_2026-03-17.md`
