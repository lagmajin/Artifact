...
public:
    float amount_=1.0f;
    bool invert_=false;
    ~FindEdgesEffectGPUImpl() override {
    ...
    void applyGPU(const ImageF32x4RGBAWithCache& src, ImageF32x4RGBAWithCache& dst) override {
        ...
        Diligent::TextureDesc desc{};desc.Name="FindEdges/Input";desc.Type=Diligent::RESOURCE_DIM_TEX_2D;desc.Width=image.width();desc.Height=image.height();desc.Format=Diligent::TEX_FORMAT_RGBA32_FLOAT;desc.MipLevels=1;desc.ArraySize=1;desc.SampleCount=1;desc.Usage=Diligent::USAGE_IMMUTABLE;desc.BindFlags=Diligent::BIND_SHADER_RESOURCE;Diligent::TextureSubResData sub{};sub.pData=pixels;sub.Stride=static_cast<Diligent::Uint64>(image.width())*sizeof(float)*4ull;Diligent::TextureData init{};init.pSubResources=&sub;init.NumSubresources=1;Diligent::RefCntAutoPtr<Diligent::ITexture> input;device_->CreateTexture(desc,&init,&input);if(!input){applyCPU(src,dst);return;}
        Diligent::TextureDesc outDesc=desc;outDesc.Name="FindEdges/Output";outDesc.Usage=Diligent::USAGE_DEFAULT;outDesc.BindFlags=Diligent::BIND_SHADER_RESOURCE|Diligent::BIND_UNORDERED_ACCESS;if(!output_||output_->GetDesc().Width!=outDesc.Width||output_->GetDesc().Height!=outDesc.Height||output_->GetDesc().Format!=outDesc.Format||output_->GetDesc().BindFlags!=outDesc.BindFlags){output_.Release();device_->CreateTexture(outDesc,nullptr,&output_);}if(!output_){applyCPU(src,dst);return;}
        Diligent::BufferDesc cbDesc{};cbDesc.Name="FindEdges/Params";cbDesc.Size=sizeof(Params);cbDesc.Usage=Diligent::USAGE_DYNAMIC;cbDesc.BindFlags=Diligent::BIND_UNIFORM_BUFFER;cbDesc.CPUAccessFlags=Diligent::CPU_ACCESS_WRITE;if(!params_)device_->CreateBuffer(cbDesc,nullptr,&params_);if(!params_){applyCPU(src,dst);return;}void* mapped=nullptr;context_->MapBuffer(params_,Diligent::MAP_WRITE,Diligent::MAP_FLAG_DISCARD,mapped);if(!mapped){applyCPU(src,dst);return;}Params values{amount_,invert_?1.0f:0.0f};std::memcpy(mapped,&values,sizeof(values));context_->UnmapBuffer(params_,Diligent::MAP_WRITE);
        static Diligent::ShaderResourceVariableDesc vars[]={{Diligent::SHADER_TYPE_COMPUTE,"FindEdgesParams",Diligent::SHADER_RESOURCE_VARIABLE_TYPE_DYNAMIC},{Diligent::SHADER_TYPE_COMPUTE,"g_InputTexture",Diligent::SHADER_RESOURCE_VARIABLE_TYPE_DYNAMIC},{Diligent::SHADER_TYPE_COMPUTE,"g_OutputTexture",Diligent::SHADER_RESOURCE_VARIABLE_TYPE_DYNAMIC}};if(!executor_){gpuContext_=std::make_unique<ArtifactCore::GpuContext>(device_,context_);executor_=std::make_unique<ArtifactCore::ComputeExecutor>(*gpuContext_);}ArtifactCore::ComputePipelineDesc pipeline{};pipeline.name="FindEdges/PSO";pipeline.shaderSource=kHlsl;pipeline.entryPoint="main";pipeline.sourceLanguage=Diligent::SHADER_SOURCE_LANGUAGE_HLSL;pipeline.variables=vars;pipeline.variableCount=3;pipeline.defaultVariableType=Diligent::SHADER_RESOURCE_VARIABLE_TYPE_STATIC;if(!pipelineReady_&&(!executor_->build(pipeline)||!executor_->createShaderResourceBinding(true)||!executor_->setBuffer("FindEdgesParams",params_))){applyCPU(src,dst);return;}pipelineReady_=true;if(!executor_->setTextureView("g_InputTexture",input->GetDefaultView(Diligent::TEXTURE_VIEW_SHADER_RESOURCE))||!executor_->setTextureView("g_OutputTexture",output_->GetDefaultView(Diligent::TEXTURE_VIEW_UNORDERED_ACCESS))){applyCPU(src,dst);return;}executor_->dispatch(context_,ArtifactCore::ComputeExecutor::makeDispatchAttribs(outDesc.Width,outDesc.Height,1,8,8,1),Diligent::RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
        Diligent::TextureDesc stagingDesc=outDesc;stagingDesc.Name="FindEdges/Readback";stagingDesc.Usage=Diligent::USAGE_STAGING;stagingDesc.BindFlags=Diligent::BIND_NONE;stagingDesc.CPUAccessFlags=Diligent::CPU_ACCESS_READ;if(!staging_||staging_->GetDesc().Width!=stagingDesc.Width||staging_->GetDesc().Height!=stagingDesc.Height||staging_->GetDesc().Format!=stagingDesc.Format){staging_.Release();device_->CreateTexture(stagingDesc,nullptr,&staging_);}if(!staging_){applyCPU(src,dst);return;}Diligent::CopyTextureAttribs copy(output_,Diligent::RESOURCE_STATE_TRANSITION_MODE_TRANSITION,staging_,Diligent::RESOURCE_STATE_TRANSITION_MODE_TRANSITION);context_->CopyTexture(copy);context_->Flush();context_->WaitForIdle();Diligent::MappedTextureSubresource read{};context_->MapTextureSubresource(staging_,0,0,Diligent::MAP_READ,Diligent::MAP_FLAG_NONE,nullptr,read);if(!read.pData||!read.Stride){applyCPU(src,dst);return;}cv::Mat result(static_cast<int>(outDesc.Height),static_cast<int>(outDesc.Width),CV_32FC4,read.pData,read.Stride);dst.image().setFromCVMat(result,image.colorDescriptor());context_->UnmapTextureSubresource(staging_,0,0);
    }
private:
