...
public:
    ...
    void applyGPU(const ImageF32x4RGBAWithCache& src,ImageF32x4RGBAWithCache& dst) override {
        ...
        Diligent::TextureDesc od{};od.Name="HexGrid/Output";od.Type=Diligent::RESOURCE_DIM_TEX_2D;od.Width=image.width();od.Height=image.height();od.Format=Diligent::TEX_FORMAT_RGBA32_FLOAT;od.MipLevels=1;od.ArraySize=1;od.SampleCount=1;od.Usage=Diligent::USAGE_DEFAULT;od.BindFlags=Diligent::BIND_SHADER_RESOURCE|Diligent::BIND_UNORDERED_ACCESS;if(!outputTex_||outputTex_->GetDesc().Width!=od.Width||outputTex_->GetDesc().Height!=od.Height||outputTex_->GetDesc().Format!=od.Format||outputTex_->GetDesc().BindFlags!=od.BindFlags){outputTex_.Release();device->CreateTexture(od,nullptr,&outputTex_);}if(!outputTex_){cpuFallback(src,dst);return;}
