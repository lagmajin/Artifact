...
[numthreads(8,8,1)] void main(uint3 dtid:SV_DispatchThreadID){ uint w,h; g_OutputTexture.GetDimensions(w,h); if(dtid.x>=w||dtid.y>=h) return; int2 p=int2(dtid.xy); float4 px=g_InputTexture[dtid.xy]; float3 c=px.rgb; float lum=luma(c); float softThreshold=max(0,g_Threshold-g_ThresholdSoftness*(1-g_Threshold)); float highlight=saturate((lum-softThreshold)/max(0.0001f,1-softThreshold)); float edge=0.5f*(abs(sampleLuma(p+int2(1,0),w,h)-sampleLuma(p-int2(1,0),w,h))+abs(sampleLuma(p+int2(0,1),w,h)-sampleLuma(p-int2(0,1),w,h))); float glow=saturate(highlight+edge*g_EdgeBoost); float3 bloom=lerp(float3(glow,glow,glow), c*glow, g_TintMix); px.rgb=saturate(c + bloom * g_Amount); g_OutputTexture[dtid.xy]=px; }
)";
