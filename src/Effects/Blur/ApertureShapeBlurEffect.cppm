...
std::vector<AbstractProperty> ApertureShapeBlurEffect::getProperties() const {
    ...
    auto& radius = props.emplace_back(); radius.setName("Radius"); radius.setDisplayLabel(QStringLiteral("Radius")); radius.setType(PropertyType::Float); radius.setValue(radius_); radius.setDefaultValue(18.0); radius.setMinValue(QVariant(0.0)); radius.setMaxValue(QVariant(256.0)); radius.setHardRange(0.0, 256.0); radius.setSoftRange(0.0, 64.0); radius.setStep(0.1); radius.setUnit(QStringLiteral("px")); radius.setTooltip(QStringLiteral("Radius of the aperture point-spread function."));
    auto& shape = props.emplace_back(); shape.setName("Shape"); shape.setDisplayLabel(QStringLiteral("Aperture Shape")); shape.setType(PropertyType::Integer); shape.setValue(shape_); shape.setDefaultValue(0); shape.setMinValue(QVariant(0)); shape.setMaxValue(QVariant(3)); shape.setHardRange(0, 3); shape.setTooltip(QStringLiteral("0=Circle, 1=Pentagon, 2=Hexagon, 3=Custom PSF image."));
    auto& rotation = props.emplace_back(); rotation.setName("Rotation"); rotation.setDisplayLabel(QStringLiteral("Aperture Rotation")); rotation.setType(PropertyType::Float); rotation.setValue(rotation_); rotation.setDefaultValue(0.0); rotation.setMinValue(QVariant(-180.0)); rotation.setMaxValue(QVariant(180.0)); rotation.setHardRange(-180.0, 180.0); rotation.setStep(0.1); rotation.setUnit(QStringLiteral("deg")); rotation.setTooltip(QStringLiteral("Rotation of the aperture shape in degrees."));
    auto& edge = props.emplace_back(); edge.setName("Edge Brightness"); edge.setDisplayLabel(QStringLiteral("Edge Brightness")); edge.setType(PropertyType::Float); edge.setValue(edgeBrightness_); edge.setDefaultValue(0.2); edge.setMinValue(QVariant(0.0)); edge.setMaxValue(QVariant(3.0)); edge.setHardRange(0.0, 3.0); edge.setStep(0.01); edge.setTooltip(QStringLiteral("Brightens the outer rim of the aperture response."));
    auto& boost = props.emplace_back(); boost.setName("Highlight Boost"); boost.setDisplayLabel(QStringLiteral("Highlight Boost")); boost.setType(PropertyType::Float); boost.setValue(highlightBoost_); boost.setDefaultValue(0.35); boost.setMinValue(QVariant(0.0)); boost.setMaxValue(QVariant(4.0)); boost.setHardRange(0.0, 4.0); boost.setStep(0.01); boost.setTooltip(QStringLiteral("Boosts bright detail before bokeh convolution."));
    auto& path = props.emplace_back(); path.setName("PSF Image Path"); path.setDisplayLabel(QStringLiteral("Custom PSF Image")); path.setType(PropertyType::String); path.setValue(psfImagePath_); path.setDefaultValue(QString()); path.setTooltip(QStringLiteral("Path to a custom aperture point-spread function image."));
    return props;
}
...
void ApertureShapeBlurEffect::setPropertyValue(const UniString& name,
    ...
    else if (setCommonPropertyValue(key, value)) return;
    syncImpl();
}
