...
public:
    ...
    void applyGPU(const ImageF32x4RGBAWithCache& src, ImageF32x4RGBAWithCache& dst) override {
        ...
        if (!pipelineReady_) {
            ...
            if (!executor_->build(desc) || !executor_->createShaderResourceBinding(true) || !executor_->setBuffer("KaleidoscopeParams", paramsCB_)) { applyCPU(src, dst); return; }
            pipelineReady_ = true;
        }
        ...
        if (!readbackTexture(device_, context_, outputTex_, dst, src.image().colorDescriptor())) { applyCPU(src, dst); return; }
...
void main(uint3 dtid : SV_DispatchThreadID) {
    ...
    static bool readbackTexture(Diligent::IRenderDevice* device, Diligent::IDeviceContext* ctx, Diligent::ITexture* src, ImageF32x4RGBAWithCache& dst, const ArtifactCore::SurfaceColorDescriptor& colorDescriptor) {
