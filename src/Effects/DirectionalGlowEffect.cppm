...
// ResidentGenericParams). Mapping: P0 threshold, P1 intensity, P2 length1,
// P3 length2, P4 weight1, P5 weight2, P6 pattern, P7 angleOffset.
    static constexpr const char* kDirectionalGlowResidentHlsl=R"(Texture2D<float4> g_InputTexture:register(t0);RWTexture2D<float4> g_OutputTexture:register(u0);float4 sampleBright(int2 p,uint2 size){p=clamp(p,int2(0,0),int2(size)-1);float4 v=g_InputTexture[p];float lum=dot(v.rgb,float3(0.114,0.587,0.299));return lum>g_P0?v*((lum-g_P0)/max(0.001,1.0-g_P0)):0;}float3 streak(int2 p,uint2 size,float angle,float len){float2 d=float2(cos(angle),sin(angle));int radius=(int)max(1.0,len*.5);float3 sum=0;float total=0;for(int s=-128;s<=128;++s){if(abs(s)>radius)continue;int2 q=p+int2(round(d*s));float w=exp(-.5*s*s/max(1.0,radius*radius*.25));sum+=sampleBright(q,size).rgb*w;total+=w;}return total>0?sum/total:0;}[numthreads(8,8,1)]void main(uint3 id:SV_DispatchThreadID){uint w,h;g_OutputTexture.GetDimensions(w,h);if(id.x>=w||id.y>=h)return;int2 p=int2(id.xy);float a=g_P7*0.0174532925;float3 s=streak(p,uint2(w,h),a,g_P2)*g_P4+streak(p,uint2(w,h),a,g_P3)*g_P5;if(g_P6>0.5&&g_P6<1.5)s+=streak(p,uint2(w,h),a+1.5707963,g_P2)*g_P4+streak(p,uint2(w,h),a+1.5707963,g_P3)*g_P5;if(g_P6>1.5&&g_P6<2.5)for(int k=1;k<4;k+=1){float ang=a+1.5707963*k*.5;s+=streak(p,uint2(w,h),ang,g_P2)*g_P4+streak(p,uint2(w,h),ang,g_P3)*g_P5;}float4 px=g_InputTexture[id.xy];px.rgb=saturate(px.rgb+s*g_P1);g_OutputTexture[id.xy]=px;})";
public:
    static constexpr const char* residentHlsl() { return kDirectionalGlowResidentHlsl; }
private:
...
DirectionalGlowEffect::DirectionalGlowEffect() {
    ...
    registerGpuGenericShader(
        DirectionalGlowEffect::kGpuGenericKey,
        GpuGenericShaderRecord{
            DirectionalGlowGPUImpl::residentHlsl(), "main", GpuGenericResourceKind::Filter});
}
