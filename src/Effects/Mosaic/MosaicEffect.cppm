...
public:
    ...
    void applyGPU(const ImageF32x4RGBAWithCache& src, ImageF32x4RGBAWithCache& dst) override {
        ...
        Diligent::TextureDesc outDesc=inputTex->GetDesc(); outDesc.Usage=Diligent::USAGE_DEFAULT; outDesc.BindFlags=Diligent::BIND_UNORDERED_ACCESS | Diligent::BIND_SHADER_RESOURCE; outDesc.Name="Mosaic/OutputTexture"; if(!outputTex_||outputTex_->GetDesc().Width!=outDesc.Width||outputTex_->GetDesc().Height!=outDesc.Height||outputTex_->GetDesc().Format!=outDesc.Format||outputTex_->GetDesc().BindFlags!=outDesc.BindFlags){outputTex_.Release();device_->CreateTexture(outDesc,nullptr,&outputTex_);} if (!outputTex_) { applyCPU(src,dst); return; }
        ...
        if (!readbackTexture(device_, context_, outputTex_, dst, "Mosaic/StagingTexture", src.image().colorDescriptor())) { applyCPU(src,dst); return; }
    }
public:
...
[numthreads(8,8,1)] void main(uint3 dtid:SV_DispatchThreadID){ uint w,h; g_OutputTexture.GetDimensions(w,h); if(dtid.x>=w||dtid.y>=h) return; int cell=max(1,(int)g_CellSize); int2 p=int2(dtid.xy); int2 cellOrigin=int2((p.x/cell)*cell, (p.y/cell)*cell); int2 cellMax=int2(min(cellOrigin.x+cell,(int)w), min(cellOrigin.y+cell,(int)h)); int2 c=(cellOrigin+cellMax)/2; int radius=max(1,min(cellMax.x-cellOrigin.x, cellMax.y-cellOrigin.y)/2); float4 sum=0; int count=0; for(int y=cellOrigin.y;y<cellMax.y;++y){ for(int x=cellOrigin.x;x<cellMax.x;++x){ if(g_ShapeMode <= 0.5f || abs(x-c.x)+abs(y-c.y) <= radius){ sum += g_InputTexture[int2(x,y)]; count++; } }} float4 avg = sum / max(count,1); if(g_ShapeMode > 0.5f){ if(abs(p.x-c.x)+abs(p.y-c.y) <= radius) g_OutputTexture[dtid.xy]=avg; else g_OutputTexture[dtid.xy]=g_InputTexture[dtid.xy]; } else { g_OutputTexture[dtid.xy]=avg; } }
)";
...
void MosaicEffect::setCellSize(float v) { cellSize_ = std::isfinite(v) ? std::max(1.0f, v) : 8.0f; syncImpls(); }
