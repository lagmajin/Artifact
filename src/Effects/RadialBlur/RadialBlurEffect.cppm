...
public:
    ~RadialBlurEffectGPUImpl() override {
        if (context_) { context_->Flush(); context_->WaitForIdle(); }
        executor_.reset(); gpuContext_.reset(); params_.Release(); context_.Release(); device_.Release();
        if (usingSharedDevice_) releaseSharedRenderDevice();
    }
    ...
    void applyGPU(const ImageF32x4RGBAWithCache& src, ImageF32x4RGBAWithCache& dst) override {
        if (!device_ || !context_) { if (!acquireSharedRenderDeviceForCurrentBackend(device_, context_)) { applyCPU(src, dst); return; } usingSharedDevice_ = true; }
        ...
        Diligent::TextureSubResData sub{};sub.pData=data;sub.Stride=static_cast<Diligent::Uint64>(image.width())*sizeof(float)*4ull;Diligent::TextureData init{};init.pSubResources=&sub;init.NumSubresources=1;Diligent::RefCntAutoPtr<Diligent::ITexture> input;device_->CreateTexture(d,&init,&input);if(!input){applyCPU(src,dst);return;}
        auto od=d;od.Name="RadialBlur/Output";od.Usage=Diligent::USAGE_DEFAULT;od.BindFlags=Diligent::BIND_SHADER_RESOURCE|Diligent::BIND_UNORDERED_ACCESS;if(!output_||output_->GetDesc().Width!=od.Width||output_->GetDesc().Height!=od.Height||output_->GetDesc().Format!=od.Format||output_->GetDesc().BindFlags!=od.BindFlags){output_.Release();device_->CreateTexture(od,nullptr,&output_);}if(!output_){applyCPU(src,dst);return;}
        struct Params{float amount,cx,cy,samples;int type;float pad[3];};Diligent::BufferDesc bd{};bd.Name="RadialBlur/Params";bd.Size=sizeof(Params);bd.Usage=Diligent::USAGE_DYNAMIC;bd.BindFlags=Diligent::BIND_UNIFORM_BUFFER;bd.CPUAccessFlags=Diligent::CPU_ACCESS_WRITE;if(!params_)device_->CreateBuffer(bd,nullptr,&params_);if(!params_){applyCPU(src,dst);return;}void*m=nullptr;context_->MapBuffer(params_,Diligent::MAP_WRITE,Diligent::MAP_FLAG_DISCARD,m);if(!m){applyCPU(src,dst);return;}Params p{cpuImpl_.amount_,cpuImpl_.centerX_,cpuImpl_.centerY_,static_cast<float>(cpuImpl_.samples_),cpuImpl_.type_,{}};std::memcpy(m,&p,sizeof(p));context_->UnmapBuffer(params_,Diligent::MAP_WRITE);
        static Diligent::ShaderResourceVariableDesc vars[]={{Diligent::SHADER_TYPE_COMPUTE,"RadialBlurParams",Diligent::SHADER_RESOURCE_VARIABLE_TYPE_DYNAMIC},{Diligent::SHADER_TYPE_COMPUTE,"g_InputTexture",Diligent::SHADER_RESOURCE_VARIABLE_TYPE_DYNAMIC},{Diligent::SHADER_TYPE_COMPUTE,"g_OutputTexture",Diligent::SHADER_RESOURCE_VARIABLE_TYPE_DYNAMIC}};if(!executor_){gpuContext_=std::make_unique<ArtifactCore::GpuContext>(device_,context_);executor_=std::make_unique<ArtifactCore::ComputeExecutor>(*gpuContext_);}ArtifactCore::ComputePipelineDesc pd{};pd.name="RadialBlur/PSO";pd.shaderSource=kHlsl;pd.entryPoint="main";pd.sourceLanguage=Diligent::SHADER_SOURCE_LANGUAGE_HLSL;pd.variables=vars;pd.variableCount=3;pd.defaultVariableType=Diligent::SHADER_RESOURCE_VARIABLE_TYPE_STATIC;if(!pipelineReady_&&(!executor_->build(pd)||!executor_->createShaderResourceBinding(true)||!executor_->setBuffer("RadialBlurParams",params_))){applyCPU(src,dst);return;}pipelineReady_=true;if(!executor_->setTextureView("g_InputTexture",input->GetDefaultView(Diligent::TEXTURE_VIEW_SHADER_RESOURCE))||!executor_->setTextureView("g_OutputTexture",output_->GetDefaultView(Diligent::TEXTURE_VIEW_UNORDERED_ACCESS))){applyCPU(src,dst);return;}executor_->dispatch(context_,ArtifactCore::ComputeExecutor::makeDispatchAttribs(od.Width,od.Height,1,8,8,1),Diligent::RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
        auto sd=od;sd.Name="RadialBlur/Readback";sd.Usage=Diligent::USAGE_STAGING;sd.BindFlags=Diligent::BIND_NONE;sd.CPUAccessFlags=Diligent::CPU_ACCESS_READ;if(!staging_||staging_->GetDesc().Width!=sd.Width||staging_->GetDesc().Height!=sd.Height||staging_->GetDesc().Format!=sd.Format){staging_.Release();device_->CreateTexture(sd,nullptr,&staging_);}if(!staging_){applyCPU(src,dst);return;}context_->CopyTexture(Diligent::CopyTextureAttribs(output_,Diligent::RESOURCE_STATE_TRANSITION_MODE_TRANSITION,staging_,Diligent::RESOURCE_STATE_TRANSITION_MODE_TRANSITION));context_->Flush();context_->WaitForIdle();Diligent::MappedTextureSubresource read{};context_->MapTextureSubresource(staging_,0,0,Diligent::MAP_READ,Diligent::MAP_FLAG_NONE,nullptr,read);if(!read.pData||!read.Stride){applyCPU(src,dst);return;}cv::Mat result(image.height(),image.width(),CV_32FC4,read.pData,read.Stride);dst.image().setFromCVMat(result,image.colorDescriptor());context_->UnmapTextureSubresource(staging_,0,0);
    }
private:
