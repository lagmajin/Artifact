...
namespace Artifact
{
    ...
    public:
        ...
        QString audioChannelMode;       // "source", "mono", "stereo", "5.1", "7.1"
    ...
    public:
            ...
            for (int i = 0; i < jobs.size(); ++i) {
                if (jobs[i].status == ArtifactRenderJob::Status::Pending) {
                    ...
                    if (!patternedOutput && (QFileInfo(output).isFile() || reservedOutputs.contains(output))) {
                        ...
                        QRegularExpression versionPattern(QStringLiteral("_v(\\d+)$"));
                        const auto match = versionPattern.match(stem);
                        int version = match.hasMatch() ? match.captured(1).toInt() + 1 : 1;
                        if (match.hasMatch()) stem = stem.left(match.capturedStart());
                        QString versioned;
                        do {
        ...
        QString jobWorkerPoolAt(int index) const {
            if (index < 0 || index >= jobs.size()) return {};
            return jobs[index].workerPool;
        }

        void setJobWorkerPool(int index, const QString& pool) {
            if (index < 0 || index >= jobs.size()) return;
            jobs[index].workerPool = pool.trimmed().left(128);
            if (jobUpdated) jobUpdated(index);
        }
    ...
    public:
        ...
        void processFramesForJob(
            ...
            bool isVideo,
            bool isHtmlPlayer,
            const QString& videoRenderPath,
            const QDir& outDir,
            const QFileInfo& outInfo,
            IVideoEncodeBackend* videoBackend,
        ...
        void persistQueueState() {
            ...
            const QJsonObject root{
                ...
                {QStringLiteral("history"), owner_->completedJobHistory()}
            };
    ...
    int ArtifactRenderQueueService::cancelRenderQueuesAt(const QList<int>& indices) {
        std::set<int> uniqueIndices;
        for (const int index : indices) {
            if (index >= 0 && index < impl_->queueManager.jobCount())
                uniqueIndices.insert(index);
        ...
        for (const int index : uniqueIndices) {
            ...
            if (status == ArtifactRenderJob::Status::Pending
                || status == ArtifactRenderJob::Status::Rendering) {
                impl_->queueManager.cancelRendering(index);
                ++changed;
            }
        }
    ...
    QString ArtifactRenderQueueService::jobWorkerPoolAt(int index) const {
        return impl_->queueManager.jobWorkerPoolAt(index);
    }

    void ArtifactRenderQueueService::setJobWorkerPoolAt(
        int index, const QString& pool) {
        impl_->queueManager.setJobWorkerPool(index, pool);
        impl_->syncCoreQueueModel();
    }
    ...
    void ArtifactRenderQueueService::Impl::processFramesForJob(
        ArtifactRenderQueueService* service,
        int jobIndex,
        const ArtifactRenderJob& job,
        const ArtifactCompositionPtr& compositionForRender,
        ...
        for (int f = consumerStartF; f < endF; ++f) {
            FrameRenderOutput frameOutput;
            {
                ...
                const auto currentJobStatus = queueManager.getJob(jobIndex).status;
            ...
            if (isVideo) {
                ...
                const QString frameExt = sequenceExtension(job.outputFormat, job.codec);
                QString baseName = outInfo.completeBaseName();
                if (baseName.isEmpty()) {
                    baseName = QStringLiteral("render");
                }
    ...
    QJsonArray ArtifactRenderQueueService::completedJobHistory() const {
        QJsonArray result;
    ...
    void ArtifactRenderQueueService::fromJson(const QJsonArray& arr) {
        ...
        for (const auto& val : arr) {
            ...
            if (job.audioChannelMode != QStringLiteral("source")
                ...
                && job.audioChannelMode != QStringLiteral("7.1")) {
                job.audioChannelMode = QStringLiteral("stereo");
            }
            if (job.audioSampleRate != 0 && job.audioSampleRate != 48000
                && job.audioSampleRate != 96000) {
