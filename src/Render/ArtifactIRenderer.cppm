...
namespace Artifact
{
 ...
 namespace {
  ...
  Detail::float2 toDetailFloat2(Diligent::float2 value)
  {
   return { value.x, value.y };
  }
 ...
 public:
  ...
  void zoomAroundViewportPoint(Detail::float2 viewportPos, float newZoom) { viewport_.ZoomAroundViewportPoint(toDiligentFloat2(viewportPos), newZoom); }
  Detail::float2 canvasToViewport(Detail::float2 pos) const { return toDetailFloat2(viewport_.CanvasToViewport(toDiligentFloat2(pos))); }
  Detail::float2 viewportToCanvas(Detail::float2 pos) const { return toDetailFloat2(viewport_.ViewportToCanvas(toDiligentFloat2(pos))); }
 ...
 ArtifactIRenderer::Impl::Impl(RefCntAutoPtr<IRenderDevice> device, RefCntAutoPtr<IDeviceContext>& context, QWidget* widget)
  : pDevice_(device), pImmediateContext_(context), widget_(widget)
 {
...
void ArtifactIRenderer::Impl::createSwapChain(QWidget* window)
 ...
 void ArtifactIRenderer::Impl::drawThickLineLocal(float2 p1, float2 p2, float thickness, const FloatColor& color)
 {
  ...
  void ArtifactIRenderer::zoomAroundViewportPoint(Detail::float2 viewportPos, float newZoom) { impl_->zoomAroundViewportPoint(viewportPos, newZoom); }

  Detail::float2 ArtifactIRenderer::canvasToViewport(Detail::float2 pos) const { return impl_->canvasToViewport(pos); }
  Detail::float2 ArtifactIRenderer::viewportToCanvas(Detail::float2 pos) const { return impl_->viewportToCanvas(pos); }
...
void ArtifactIRenderer::drawBezierLocal(Detail::float2 p0, Detail::float2 p1, Detail::float2 p2, float thickness, const FloatColor& color) {
    impl_->drawBezierLocal(toDiligentFloat2(p0), toDiligentFloat2(p1), toDiligentFloat2(p2), thickness, color);
}
...
void ArtifactIRenderer::drawBezierLocal(Detail::float2 p0, Detail::float2 p1, Detail::float2 p2, Detail::float2 p3, float thickness, const FloatColor& color) {
    impl_->drawBezierLocal(toDiligentFloat2(p0), toDiligentFloat2(p1), toDiligentFloat2(p2), toDiligentFloat2(p3), thickness, color);
}

void ArtifactIRenderer::drawSolidTriangleLocal(Detail::float2 p0, Detail::float2 p1, Detail::float2 p2, const FloatColor& color) {
    impl_->drawSolidTriangleLocal(toDiligentFloat2(p0), toDiligentFloat2(p1), toDiligentFloat2(p2), color);
}
