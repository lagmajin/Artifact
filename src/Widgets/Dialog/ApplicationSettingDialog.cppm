...
QList<SettingItemInfo> GeneralSettingPage::searchableItems() const
{
  ...
  if (impl_->autoSaveIntervalSpinBox_) {
    items.append({QStringLiteral("Save interval"), QStringLiteral("Auto-save interval in minutes"),
                  QStringLiteral("General"), impl_->autoSaveIntervalSpinBox_});
  }
  if (impl_->showStartupDialogCheckBox_) {
    items.append({QStringLiteral("Load last project on startup"),
                  QStringLiteral("Restore the last project when the app starts"),
...
QList<SettingItemInfo> ImportSettingPage::searchableItems() const
{
  ...
  if (impl_->defaultFrameRateCombo_) {
    items.append({QStringLiteral("Default frame rate"), QStringLiteral("Default import frame rate"),
                  QStringLiteral("Import"), impl_->defaultFrameRateCombo_});
  }
  if (impl_->colorSpaceCombo_) {
    items.append({QStringLiteral("Color space"), QStringLiteral("Default import color space"),
                  QStringLiteral("Import"), impl_->colorSpaceCombo_});
  }
  if (impl_->audioSampleRateCombo_) {
    items.append({QStringLiteral("Audio sample rate"), QStringLiteral("Default import audio sample rate"),
                  QStringLiteral("Import"), impl_->audioSampleRateCombo_});
  }
  if (impl_->autoDetectAlphaCheckBox_) {
    items.append({QStringLiteral("Auto-detect alpha channel"), QStringLiteral("Detect alpha during import"),
  ...
  if (impl_->interpretFootageCheckBox_) {
    items.append({QStringLiteral("Interpret footage on import"),
                  QStringLiteral("Interpret footage settings when importing"),
                  QStringLiteral("Import"), impl_->interpretFootageCheckBox_});
  }
  if (impl_->fieldOrderCombo_) {
    items.append({QStringLiteral("Field order"), QStringLiteral("Default field order for footage"),
                  QStringLiteral("Import"), impl_->fieldOrderCombo_});
  }
  if (impl_->stillDurationSpinBox_) {
    items.append({QStringLiteral("Still image duration"), QStringLiteral("Default duration for still images"),
  ...
  if (impl_->createCompositionCheckBox_) {
    items.append({QStringLiteral("Create composition when importing sequences"),
                  QStringLiteral("Automatically create a composition for sequences"),
...
QList<SettingItemInfo> PreviewSettingPage::searchableItems() const
{
  ...
  if (impl_->previewQualityCombo_) {
    items.append({QStringLiteral("Preview quality preset"), QStringLiteral("Preview render quality preset"),
                  QStringLiteral("Preview"), impl_->previewQualityCombo_});
  }
  if (impl_->previewResolutionSlider_) {
    items.append({QStringLiteral("Preview resolution"), QStringLiteral("Preview resolution percentage"),
                  QStringLiteral("Preview"), impl_->previewResolutionSlider_});
  }
  if (impl_->enableCacheCheckBox_) {
    items.append({QStringLiteral("Enable RAM cache"), QStringLiteral("Toggle RAM cache"),
                  QStringLiteral("Preview"), impl_->enableCacheCheckBox_});
  }
  if (impl_->cacheSizeSpinBox_) {
    items.append({QStringLiteral("Cache size"), QStringLiteral("RAM cache size in MB"),
                  QStringLiteral("Preview"), impl_->cacheSizeSpinBox_});
  }
  if (impl_->enableDiskCacheCheckBox_) {
    items.append({QStringLiteral("Enable disk cache"), QStringLiteral("Toggle disk cache"),
                  QStringLiteral("Preview"), impl_->enableDiskCacheCheckBox_});
  }
  if (impl_->generateThumbnailsCheckBox_) {
    items.append({QStringLiteral("Generate thumbnails"), QStringLiteral("Generate thumbnails for media"),
                  QStringLiteral("Preview"), impl_->generateThumbnailsCheckBox_});
  }
  if (impl_->thumbnailQualityCombo_) {
    items.append({QStringLiteral("Thumbnail quality"), QStringLiteral("Thumbnail generation quality"),
                  QStringLiteral("Preview"), impl_->thumbnailQualityCombo_});
  }
  if (impl_->enableGPUCheckBox_) {
    items.append({QStringLiteral("Enable GPU acceleration"), QStringLiteral("Toggle GPU usage"),
                  QStringLiteral("Preview"), impl_->enableGPUCheckBox_});
  }
  if (impl_->gpuDeviceCombo_) {
    items.append({QStringLiteral("GPU device"), QStringLiteral("Preferred GPU device"),
...
QList<SettingItemInfo> ShortcutSettingPage::searchableItems() const
{
  QList<SettingItemInfo> items;
  if (impl_->filterEdit_) {
    items.append({QStringLiteral("Shortcut filter"), QStringLiteral("Filter shortcut list"),
                  QStringLiteral("Shortcuts"), impl_->filterEdit_});
  }
  if (impl_->shortcutTable_) {
    items.append({QStringLiteral("Shortcut table"), QStringLiteral("Shortcut assignments table"),
...
{
    QList<SettingItemInfo> items;
    if (impl_->memoryUsageBar_) {
        items.append({QStringLiteral("Memory usage"), QStringLiteral("Current system memory usage"),
                      QStringLiteral("Memory & Performance"), impl_->memoryUsageBar_});
    }
    if (impl_->cpuUsageBar_) {
        items.append({QStringLiteral("CPU usage"), QStringLiteral("Current process CPU usage"),
                      QStringLiteral("Memory & Performance"), impl_->cpuUsageBar_});
    }
    if (impl_->workerThreadsSpinBox_) {
        items.append({QStringLiteral("Worker threads"), QStringLiteral("Render worker thread count"),
                      QStringLiteral("Memory & Performance"), impl_->workerThreadsSpinBox_});
    }
    if (impl_->autoTuneButton_) {
        items.append({QStringLiteral("Auto-tune"), QStringLiteral("Auto tune worker threads"),
                      QStringLiteral("Memory & Performance"), impl_->autoTuneButton_});
    }
    if (impl_->clearCacheButton_) {
        items.append({QStringLiteral("Clear cache"), QStringLiteral("Clear application caches"),
                      QStringLiteral("Memory & Performance"), impl_->clearCacheButton_});
    }
    return items;
}
...
// PluginSettingPage Implementation 
 ...
 W_OBJECT_IMPL(AISettingPage)
  ...
  QList<SettingItemInfo> AISettingPage::searchableItems() const
  {
   ...
   if (impl_->providerCombo_) {
    items.append({QStringLiteral("AI provider"), QStringLiteral("AI backend provider"),
                  QStringLiteral("AI"), impl_->providerCombo_});
   }
   if (impl_->modelPathEdit_) {
    items.append({QStringLiteral("AI model path"), QStringLiteral("Path to GGUF model file"),
                  QStringLiteral("AI"), impl_->modelPathEdit_});
   }
   if (impl_->browseButton_) {
    items.append({QStringLiteral("Browse model"), QStringLiteral("Browse for AI model file"),
                  QStringLiteral("AI"), impl_->browseButton_});
   }
   if (impl_->initButton_) {
    items.append({QStringLiteral("Initialize AI"), QStringLiteral("Initialize AI runtime"),
                  QStringLiteral("AI"), impl_->initButton_});
   }
   if (impl_->shutdownButton_) {
    items.append({QStringLiteral("Shutdown AI"), QStringLiteral("Shutdown AI runtime"),
                  QStringLiteral("AI"), impl_->shutdownButton_});
   }
   return items;
  }
 ...
 QList<SettingItemInfo> PluginSettingPage::searchableItems() const
 {
  ...
  if (impl_->pluginTable_) {
   items.append({QStringLiteral("Installed plugins"), QStringLiteral("Installed plugin list"),
                 QStringLiteral("Plugins"), impl_->pluginTable_});
  }
  if (impl_->refreshButton_) {
   items.append({QStringLiteral("Refresh plugins"), QStringLiteral("Reload plugin list"),
                 QStringLiteral("Plugins"), impl_->refreshButton_});
  }
  if (impl_->openFolderButton_) {
   items.append({QStringLiteral("Open plugin folder"), QStringLiteral("Open plugin directory"),
