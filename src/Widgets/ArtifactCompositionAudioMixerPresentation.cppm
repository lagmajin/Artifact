...
std::unique_ptr<UndoCommand> createAudioMixerSnapshotUndoCommand(
    ...
    const QJsonObject& before, const QJsonObject& after)
{
  return std::make_unique<AudioMixerSnapshotUndoCommand>(mixer, before, after);
}

QWidget* createAudioMixerMasterRow(AudioMixerMasterBus* masterBus, QWidget* parent)
{
  return new AudioMixerMasterRow(masterBus, parent);
}

QWidget* createAudioMixerStripRow(AudioMixerChannelStrip* strip,
                                  QWidget* parent,
                                  ArtifactCompositionAudioMixerWidget* owner)
{
  return new AudioMixerStripRow(strip, parent, owner);
}
