 ...
 : QWidget(parent), impl_(new Impl())
{
 ...
 impl_->topToolbar_->setStyleSheet("QToolBar { background: #252526; border-bottom: 1px solid #1e1e1e; spacing: 2px; }");
 ...
 impl_->bottomBar_->setStyleSheet("background: #252526; border-top: 1px solid #1e1e1e;");
 ...
 impl_->resolutionCombo_->setStyleSheet("QComboBox { background: #333; color: #ccc; border: 1px solid #444; font-size: 11px; }");
 ...
 impl_->fastPreviewBtn_->setStyleSheet("QToolButton { color: #ffeb3b; font-weight: bold; background: transparent; border: none; } QToolButton:hover { background: #444; }");
 ...
 impl_->displayOptionsBtn_->setStyleSheet("QToolButton { color: #ccc; background: transparent; border: none; } QToolButton:hover { background: #444; }");
 ...
 QObject::connect(gridAct, &QAction::toggled, this, [this](bool checked) {
  if (impl_->renderController_) impl_->renderController_->setShowGrid(checked);
 });
 QObject::connect(guidesAct, &QAction::toggled, this, [this](bool checked) {
  if (impl_->renderController_) impl_->renderController_->setShowGuides(checked);
 });
 QObject::connect(safeMarginsAct, &QAction::toggled, this, [this](bool checked) {
  if (impl_->renderController_) impl_->renderController_->setShowSafeMargins(checked);
 });
 ...
 QObject::connect(impl_->resolutionCombo_, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this](int index) {
     qDebug() << "Resolution changed to:" << impl_->resolutionCombo_->currentText();
 });
