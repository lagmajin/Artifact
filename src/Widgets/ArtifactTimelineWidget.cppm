...
bool selectedMarkersFormFlatArea(
    const QVector<ArtifactTimelineTrackPainterView::KeyframeMarkerVisual>& allMarkers,
    const QVector<ArtifactTimelineTrackPainterView::KeyframeMarkerVisual>& selectedMarkers)
{
  if (selectedMarkers.size() != 2) {
    return false;
  }
  const auto& start = selectedMarkers[0];
  const auto& end = selectedMarkers[1];
  if (start.layerId != end.layerId || start.propertyPath != end.propertyPath) {
    return false;
  }
  if (start.value.isValid() && end.value.isValid() && start.value != end.value) {
    return false;
  }

  QVector<ArtifactTimelineTrackPainterView::KeyframeMarkerVisual> laneMarkers;
  for (const auto& marker : allMarkers) {
    if (marker.layerId == start.layerId &&
        marker.propertyPath == start.propertyPath) {
      laneMarkers.push_back(marker);
    }
  }
  if (laneMarkers.size() < 2) {
    return false;
  }

  std::sort(laneMarkers.begin(), laneMarkers.end(),
            [](const auto& lhs, const auto& rhs) { return lhs.frame < rhs.frame; });
  const qint64 startFrame = static_cast<qint64>(std::llround(start.frame));
  const qint64 endFrame = static_cast<qint64>(std::llround(end.frame));
  for (int i = 0; i + 1 < laneMarkers.size(); ++i) {
    const qint64 laneStart = static_cast<qint64>(std::llround(laneMarkers[i].frame));
    const qint64 laneEnd = static_cast<qint64>(std::llround(laneMarkers[i + 1].frame));
    if (laneStart == startFrame && laneEnd == endFrame) {
      if (laneMarkers[i].value.isValid() && laneMarkers[i + 1].value.isValid() &&
          laneMarkers[i].value != laneMarkers[i + 1].value) {
        return false;
      }
      return true;
    }
...
std::optional<std::pair<LayerID, QString>> selectedAreaContext(
    const QVector<ArtifactTimelineTrackPainterView::KeyframeMarkerVisual>& allMarkers,
    const QVector<ArtifactTimelineTrackPainterView::KeyframeMarkerVisual>& selectedMarkers)
{
  if (!selectedMarkersFormFlatArea(allMarkers, selectedMarkers)) {
    return std::nullopt;
  }

  const auto& marker = selectedMarkers.front();
  if (marker.layerId.isNil()) {
    return std::nullopt;
  }
  ...
  return std::make_pair(marker.layerId, propertyPath);
}

QString selectedPropertyPathForKeyPattern(
    const ArtifactLayerTimelinePanelWrapper *layerTimelinePanel,
    const QVector<ArtifactTimelineTrackPainterView::KeyframeMarkerVisual>& allMarkers,
    const QVector<ArtifactTimelineTrackPainterView::KeyframeMarkerVisual>& selectedMarkers)
{
  if (const auto areaContext = selectedAreaContext(allMarkers, selectedMarkers)) {
    return areaContext->second;
  }
  if (layerTimelinePanel) {
    return layerTimelinePanel->currentPropertyPath().trimmed();
  }
  return {};
}

QString selectedAreaSummaryText(
    const QVector<ArtifactTimelineTrackPainterView::KeyframeMarkerVisual>& selectedMarkers)
{
  if (selectedMarkers.size() != 2) {
    return {};
  ...
  const QString label = ArtifactTimelineKeyframeModel::displayLabelForPropertyPath(
      selectedMarkers.front().propertyPath);
  return QStringLiteral("Area: %1 | F%2-F%3 | Span: %4f")
      .arg(label.isEmpty() ? QStringLiteral("Keyframe Area") : label)
      .arg(startFrame)
      .arg(endFrame)
      .arg(std::max<qint64>(1, endFrame - startFrame));
}
...
class MoveLayerToFrameCommand final : public UndoCommand {
public:
  MoveLayerToFrameCommand(CompositionID compositionId, LayerID layerId,
                          qint64 targetStart,
                          QVector<TimelineLayerStateSnapshot> beforeSnapshots)
      : compositionId_(std::move(compositionId)), layerId_(std::move(layerId)),
        targetStart_(targetStart), beforeSnapshots_(std::move(beforeSnapshots)) {}
  void undo() override {
    auto comp = safeCompositionLookup(compositionId_);
    lastOperationSucceeded_ = comp &&
        restoreTimelineLayerStateSnapshots(comp, beforeSnapshots_);
  }
  void redo() override {
    auto comp = safeCompositionLookup(compositionId_);
    if (!comp) { lastOperationSucceeded_ = false; return; }
    const auto layer = comp->layerById(layerId_);
    if (!layer) { lastOperationSucceeded_ = false; return; }
    lastOperationSucceeded_ = applyTimelineLayerMove(
        compositionId_, layerId_.toString(), targetStart_, 0.0);
  }
  QString label() const override { return QStringLiteral("Move Layer Edge"); }
  bool lastOperationSucceeded() const override { return lastOperationSucceeded_; }
private:
  CompositionID compositionId_;
  LayerID layerId_;
  qint64 targetStart_ = 0;
  QVector<TimelineLayerStateSnapshot> beforeSnapshots_;
  bool lastOperationSucceeded_ = true;
};

class TrimLayerToFrameCommand final : public UndoCommand {
public:
  TrimLayerToFrameCommand(CompositionID compositionId, LayerID layerId,
                          qint64 startFrame, qint64 durationFrame,
                          QVector<TimelineLayerStateSnapshot> beforeSnapshots)
      : compositionId_(std::move(compositionId)), layerId_(std::move(layerId)),
        startFrame_(startFrame), durationFrame_(durationFrame),
  ...
  void redo() override {
    lastOperationSucceeded_ = applyTimelineLayerTrim(
        compositionId_, layerId_.toString(), startFrame_, durationFrame_);
  }
  QString label() const override { return QStringLiteral("Trim Layer Edge"); }
  bool lastOperationSucceeded() const override { return lastOperationSucceeded_; }
private:
  CompositionID compositionId_;
  LayerID layerId_;
  qint64 startFrame_ = 0;
  qint64 durationFrame_ = 1;
  QVector<TimelineLayerStateSnapshot> beforeSnapshots_;
  bool lastOperationSucceeded_ = true;
};

class DeleteLayerAnimationCommand final : public UndoCommand {
public:
  DeleteLayerAnimationCommand(CompositionID compositionId, LayerID layerId,
                              QVector<TimelineLayerStateSnapshot> beforeSnapshots)
      : compositionId_(std::move(compositionId)), layerId_(std::move(layerId)),
        beforeSnapshots_(std::move(beforeSnapshots)) {}
  void undo() override {
  ...
  void redo() override {
    auto comp = safeCompositionLookup(compositionId_);
    const auto layer = comp ? comp->layerById(layerId_) : nullptr;
    if (!layer || layer->isTimingLocked()) { lastOperationSucceeded_ = false; return; }
    for (const auto& ref : collectAnimatablePropertyRefs({layer})) {
      if (const auto property = layer->getProperty(ref.propertyPath))
        property->clearKeyFrames();
    }
    layer->changed();
    lastOperationSucceeded_ = true;
  }
  QString label() const override { return QStringLiteral("Delete Animation"); }
  bool lastOperationSucceeded() const override { return lastOperationSucceeded_; }
private:
  CompositionID compositionId_;
  LayerID layerId_;
...
{
  if (layerIdText.trimmed().isEmpty()) return false;
  auto* svc = ArtifactProjectService::instance();
  if (!svc) return false;
  auto result = svc->findComposition(compositionId);
    ...
    const float newValue)
{
  ...
  const RationalTime newTime(newFrame, fpsInt);

  const auto keyframes = property->getKeyFrames();
  // 厳密なRationalTime比較ではなくフレーム番号で照合する。
  // Gizmo側や過去データで別スケール(例: 29と30)で書かれたキーも拾う。
  const auto it = std::find_if(
      keyframes.cbegin(), keyframes.cend(),
      [&oldKey, fpsInt](const ArtifactCore::KeyFrame& keyframe) {
        return keyframe.time.rescaledTo(fpsInt) == oldKey.frame;
      });
  if (it == keyframes.cend()) {
    return false;
  }

  // 移動先に別キーがあれば上書きせず拒否する。
  // addKeyFrameは同時刻を上書きマージするため、無検査だと隣キーが消える。
  if (newFrame != oldKey.frame) {
    const bool collides = std::any_of(
        keyframes.cbegin(), keyframes.cend(),
        [&it, &newFrame, fpsInt](const ArtifactCore::KeyFrame& keyframe) {
          return (&keyframe != &(*it)) &&
                 keyframe.time.rescaledTo(fpsInt) == newFrame;
        });
    if (collides) {
      return false;
    }
  ...
  property->removeKeyFrame(preservedKeyframe.time);
    ...
    const std::vector<CurveTrack>& widgetTracks)
{
  ...
  for (int i = 0; i < count; ++i) {
    ...
    // 別スケールで書かれたキーも拾えるよう、格納時刻をフレーム番号で照合する。
    const auto storedKeyframes = property->getKeyFrames();
    for (const int64_t frame : cachedFrames) {
      if (!widgetFrames.contains(frame)) {
        const auto stored = std::find_if(
            storedKeyframes.cbegin(), storedKeyframes.cend(),
            [frame, fpsInt](const ArtifactCore::KeyFrame& keyframe) {
              return keyframe.time.rescaledTo(fpsInt) == frame;
            });
        if (stored != storedKeyframes.cend()) {
          property->removeKeyFrame(stored->time);
        }
      }
    }
    // Insertions: frames present in the widget but missing from the cache.
    const auto afterRemovals = property->getKeyFrames();
    for (const int64_t frame : widgetFrames) {
      if (!cachedFrames.contains(frame)) {
        const bool exists = std::any_of(
            afterRemovals.cbegin(), afterRemovals.cend(),
            [frame, fpsInt](const ArtifactCore::KeyFrame& keyframe) {
              return keyframe.time.rescaledTo(fpsInt) == frame;
            });
        if (exists) {
...
public:
  ...
  quint64 gpuTimelineStaticSnapshotGeneration_ = 0;
  std::chrono::steady_clock::time_point lastGpuTimelineSnapshot_{};
  QTimer *gpuTimelineSnapshotTimer_ = nullptr;
...
void ArtifactTimelineWidget::updateCacheVisuals()
{
  ...
  if (auto* svc = ArtifactPlaybackService::instance()) {
    ...
    QString cacheToolTip =
        QStringLiteral("RAM Preview\n%1 of %2 frames playable · %3% ready")
            .arg(summary.playableFrames)
            .arg(summary.rangeFrames)
            .arg(QString::number(summary.buildRangeProgress * 100.0f, 'f', 0));
    if (summary.buildQueuePendingFrames > 0) {
      cacheToolTip += QStringLiteral("\n%1 frames building")
                          .arg(summary.buildQueuePendingFrames);
    }
    if (summary.failedFrames > 0) {
      cacheToolTip += QStringLiteral("\n%1 frames failed")
                          .arg(summary.failedFrames);
    }
    impl_->scrubBar_->setToolTip(cacheToolTip);
  }
}
...
ArtifactTimelineWidget::ArtifactTimelineWidget(QWidget *parent /*=nullptr*/)
  ...
  searchBarLayout->setContentsMargins(16, 0, 16, 0);
  ...
  auto *selectionSummaryClickFilter = new TimelineStatusClickFilter(
      selectionSummaryLabel, [this]() {
        ...
        if (targetLayerId.isNil() && currentLayer) {
          targetLayerId = currentLayer->id();
        } else if (targetLayerId.isNil() && !selectedLayers.isEmpty()) {
        ...
        if (impl_ && impl_->painterTrackView_ && !targetPropertyPath.isEmpty()) {
          impl_->painterTrackView_->setKeyframeContext(targetLayerId, targetPropertyPath);
          updateSelectionState();
          updateKeyframeState();
        }
      },
  ...
  auto addTransportButton = [headerWidget, searchBarLayout](
    ...
    button->setAccessibleDescription(
        QStringLiteral("Timeline playback control: %1").arg(toolTip));
  ...
  impl_->eventBusSubscriptions_.push_back(
      impl_->eventBus_.subscribe<TimelineScrubFinishedEvent>([this](const TimelineScrubFinishedEvent&) {
        ...
        if (impl_->gpuTimelinePreviewEnabled_) {
          // Immediately flush the final settle frame so the playhead is exact
  ...
  impl_->eventBusSubscriptions_.push_back(
      ...
      [this](const TimelineClipMoveRequestedEvent &event) {
        if (auto comp = safeCompositionLookup(impl_->compositionId_)) {
          ...
          const QVector<ArtifactAbstractLayerPtr> layers{layer};
          const auto beforeSnapshots =
              captureTimelineLayerStateSnapshots(comp, layers);
          auto cmd = std::make_unique<MoveLayerToFrameCommand>(
              impl_->compositionId_, layer->id(),
  ...
  impl_->eventBusSubscriptions_.push_back(
      ...
      [this](const TimelineClipResizeRequestedEvent &event) {
        if (auto comp = safeCompositionLookup(impl_->compositionId_)) {
          ...
          const auto beforeSnapshots =
              captureTimelineLayerStateSnapshots(comp, layers);
          auto cmd = std::make_unique<TrimLayerToFrameCommand>(
              impl_->compositionId_, layer->id(),
              static_cast<qint64>(std::llround(event.startFrame)),
  ...
  impl_->eventBusSubscriptions_.push_back(
      ...
      [this](const CurveEditorKeyDeletedEvent &event) {
                     ...
                     if (keyIndex < 0 || keyIndex >= static_cast<int>(track.keys.size())) {
                      ...
                      const double fps = timelineFrameRateFallback(composition);
                      const int64_t fpsInt = std::max<int64_t>(
                          1, static_cast<int64_t>(std::llround(fps)));
                      const int64_t targetFrame =
                          static_cast<int64_t>(std::llround(key.frame));
                      // 別スケールで書かれたキーも拾えるようフレーム番号で照合する。
                      const auto storedKeyframes = property->getKeyFrames();
                      const auto stored = std::find_if(
                          storedKeyframes.cbegin(), storedKeyframes.cend(),
                          [targetFrame, fpsInt](
                              const ArtifactCore::KeyFrame& keyframe) {
                            return keyframe.time.rescaledTo(fpsInt) ==
                                   targetFrame;
                          });
                      if (stored == storedKeyframes.cend()) {
                        return;
                      }
                      const RationalTime storedTime = stored->time;
                     const QVector<KeyframePropertyRef> refs = {
                         KeyframePropertyRef{binding.layerId, binding.propertyPath}};
                     const auto beforeSnapshots =
                         captureKeyframePropertySnapshots(composition, refs);
                      property->removeKeyFrame(storedTime);
                     layer->setDirty(LayerDirtyFlag::Property);
                     layer->changed();
                     const auto afterSnapshots =
                         captureKeyframePropertySnapshots(composition, refs);
  ...
  const QPointer<ArtifactTimelineTrackPainterView> wheelTarget(painterTrackView);
  impl_->gpuTimelineWindow_->setWheelInputHandler(
      [wheelTarget](const QPointF& position, const QPoint& angleDelta,
                    const Qt::KeyboardModifiers modifiers) {
        return wheelTarget && wheelTarget->handleNavigationWheel(
            position, angleDelta, modifiers);
      });
  const QPointer<ArtifactTimelineTrackPainterView> panTarget(painterTrackView);
  impl_->gpuTimelineWindow_->setPanInputHandler(
      [panTarget](const Qt::MouseButton button, const QPointF& position,
                  const Qt::MouseButtons buttons,
                  const Qt::KeyboardModifiers modifiers) {
        return panTarget && panTarget->handleNavigationPan(
            button, position, buttons, modifiers);
      });
  const QPointer<ArtifactTimelineTrackPainterView> interactionTarget(painterTrackView);
  impl_->gpuTimelineWindow_->setInteractionStateProvider([interactionTarget]() {
    return interactionTarget && interactionTarget->isInteracting();
  });
  ...
  impl_->gpuTimelineContainer_->setFocusPolicy(Qt::StrongFocus);
  ...
  const auto updateSmoothPlaybackPlayhead = [this]() {
    ...
    // Follow the same fractional position that is drawn, not the integer
    // frame notifications arriving between visual timer ticks.
    const double ppf = std::max(0.001, impl_->painterTrackView_->pixelsPerFrame());
    const double playheadX = visualFrame * ppf -
    ...
    if (playheadX < kFollowMargin || playheadX > followX) {
      ...
      syncTimelineHorizontalOffset(
          std::max(0.0, visualFrame * ppf - targetX));
    }
    if (impl_->navigator_) {
          ...
          [this, scrubBar, restartSmoothPlaybackPlayhead,
           ...
           requestCacheVisualRefresh](const FrameChangedEvent &event) {
            ...
            if (!isPlaying) {
              ...
              syncPlayheadOverlay();
            } else if (impl_->timeCodeWidget_) {
              impl_->timeCodeWidget_->updateTimeCode(frame.framePosition());
            }
            if (isPlaying) {
...
ArtifactTimelineWidget::~ArtifactTimelineWidget() {
  ...
  if (impl_) {
    if (impl_->gpuTimelineSnapshotTimer_) {
      impl_->gpuTimelineSnapshotTimer_->stop();
    }

    // Window containers own native QWindows.  Tear them down while this
    // QWidget and its callbacks are still valid; otherwise QWindow teardown
    // can dispatch a native parent-change event after Impl has gone away.
    const auto detachGpuWindow = [](ArtifactDiligentTimelineRenderWindow* window) {
      if (!window) {
        return;
      }
      window->setInputTarget(nullptr);
      window->setWheelInputHandler({});
      window->setPanInputHandler({});
      window->setInputUpdatedCallback({});
      window->setInteractionStateProvider({});
      QCoreApplication::removePostedEvents(window);
    };
    detachGpuWindow(impl_->gpuTimelineWindow_);
    detachGpuWindow(impl_->gpuCurveWindow_);

    delete impl_->gpuTimelineContainer_;
    impl_->gpuTimelineContainer_ = nullptr;
    impl_->gpuTimelineWindow_ = nullptr;
    delete impl_->gpuCurveContainer_;
    impl_->gpuCurveContainer_ = nullptr;
    impl_->gpuCurveWindow_ = nullptr;
  }
  delete impl_;
}
...
void ArtifactTimelineWidget::refreshTracks() {
  ...
  for (int rowIndex = 0; rowIndex < visibleRows.size(); ++rowIndex) {
    ...
    if (row.kind == TimelineRowKind::Container && composition &&
        !row.nodeId.trimmed().isEmpty()) {
      const auto childIds = composition->groupContainerChildLayerIds(row.nodeId);
      ...
      for (const auto& childId : childIds) {
        ...
        if (!child) continue;
        start = std::min(start, child->inPoint().framePosition());
        end = std::max(end, child->outPoint().framePosition());
      }
      if (start != std::numeric_limits<qint64>::max() && end > start) {
        ArtifactTimelineTrackPainterView::TrackClipVisual visual;
        visual.clipId = row.nodeId;
        visual.trackIndex = rowIndex;
        visual.startFrame = static_cast<double>(start);
...
void ArtifactTimelineWidget::keyPressEvent(QKeyEvent *event) {
  ...
  const auto zoomTimelineBy = [this](const double scale) {
    ...
    syncTimelineHorizontalOffset(anchorFrame * newPpf - anchorX);
  ...
  if ((event->key() == Qt::Key_Left || event->key() == Qt::Key_Right) &&
    ...
    if (comp && selManager) {
      ...
      auto selectedLayers = selManager->selectedLayers();
      if (selectedLayers.isEmpty()) {
        auto current = selManager->currentLayer();
        if (current) selectedLayers.insert(current);
      }

      QVector<ArtifactAbstractLayerPtr> layersVec;
      for (const auto& layer : selectedLayers) {
        if (layer && !layer->isTimingLocked()) layersVec.push_back(layer);
      }
...
void ArtifactTimelineWidget::updateKeyframeState()
{
  ...
  if (selectedArea) {
    summary += QStringLiteral(" | %1").arg(selectedAreaSummaryText(selectedMarkers));
  }
  ...
  if (impl_->easingLabButton_) {
    ...
    impl_->easingLabButton_->setToolTip(
        selectedArea
            ? QStringLiteral("Open easing tools for the selected keyframe area endpoints.")
            : QStringLiteral("Open easing tools for the selected keyframes."));
  }
  if (impl_->keyframeEaseInButton_) {
    ...
    impl_->keyframeEaseInButton_->setToolTip(
        selectedArea
            ? QStringLiteral("Apply Ease In to the selected area endpoints.")
            : QStringLiteral("Apply Ease In to the selected keyframes."));
  }
  if (impl_->keyframeEaseOutButton_) {
    ...
    impl_->keyframeEaseOutButton_->setToolTip(
        selectedArea
            ? QStringLiteral("Apply Ease Out to the selected area endpoints.")
            : QStringLiteral("Apply Ease Out to the selected keyframes."));
  }
  if (impl_->keyframeEaseInOutButton_) {
    ...
    impl_->keyframeEaseInOutButton_->setToolTip(
        selectedArea
...
void ArtifactTimelineWidget::syncGpuTimelineSnapshot()
{
  ...
  // Throttle continuous snapshot requests (e.g. during mouse scrubbing/dragging)
  // to avoid flooding the GUI thread with snapshot rebuilds (~60 FPS / 16ms cadence).
  constexpr auto kMinSnapshotInterval = std::chrono::milliseconds(16);
  const auto now = std::chrono::steady_clock::now();
  const auto elapsed = now - impl_->lastGpuTimelineSnapshot_;

  if (elapsed < kMinSnapshotInterval) {
    if (!impl_->gpuTimelineSnapshotTimer_) {
      impl_->gpuTimelineSnapshotTimer_ = new QTimer(this);
      impl_->gpuTimelineSnapshotTimer_->setSingleShot(true);
      QObject::connect(impl_->gpuTimelineSnapshotTimer_, &QTimer::timeout, this,
                       [this]() {
                         if (impl_ && impl_->gpuTimelinePreviewEnabled_) {
                           syncGpuTimelineSnapshot();
                         }
    ...
    if (!impl_->gpuTimelineSnapshotTimer_->isActive()) {
      const auto remainingMs = static_cast<int>(
          std::chrono::duration_cast<std::chrono::milliseconds>(
              kMinSnapshotInterval - elapsed).count());
      impl_->gpuTimelineSnapshotTimer_->start(std::max(1, remainingMs));
    }
    return;
  }

  if (impl_->gpuTimelineSnapshotPending_) {
    return;
  }
  impl_->gpuTimelineSnapshotPending_ = true;
  if (impl_->gpuTimelineSnapshotTimer_ && impl_->gpuTimelineSnapshotTimer_->isActive()) {
    impl_->gpuTimelineSnapshotTimer_->stop();
  }
  QMetaObject::invokeMethod(
      this,
      [this]() {
        ...
        impl_->lastGpuTimelineSnapshot_ = std::chrono::steady_clock::now();
        buildGpuTimelineSnapshot();
      },
      Qt::QueuedConnection);
}
...
void ArtifactTimelineWidget::buildGpuCurveSnapshot()
{
  ...
  if (playhead.x() >= plot.left() && playhead.x() <= plot.right()) {
    ...
    snapshot.triangles.push_back({QPointF(playhead.x() - 5.0, plot.top()),
                                  ...
                                  playheadCapColor});
    snapshot.triangles.push_back({QPointF(playhead.x() - 1.35, plot.top() + 1.0),
                                  QPointF(playhead.x() + 1.35, plot.top() + 1.0),
                                  QPointF(playhead.x(), plot.top() + 7.5),
                                  playheadColor});
  }
...
void ArtifactTimelineWidget::buildGpuTimelineSnapshot()
{
  ...
  ArtifactTimelineTrackPainterView::TrackClipVisual dragClip;
  const bool hasDragClip = view->activeDragClip(dragClip);
  ...
  // visual data are unchanged, so playback ticks and playhead seeking only pay
  // for the dynamic tail below.
  ...
  for (int frame = firstGridFrame; frame <= lastGridFrame; ++frame) {
    ...
    const QColor& color = major ? majorGridColor : mediumGridColor;
    snapshot.lines.push_back({QPointF(x, 0.0), QPointF(x, viewportHeight),
                              color, 1.0f});
  }

  for (const auto& clip : clips) {
    if (hasDragClip && clip.clipId == dragClipId) {
      continue;
    }
    ...
    const bool isRelationship =
        clip.kind == ArtifactTimelineTrackPainterView::TrackClipVisual::Kind::Transition ||
        clip.kind == ArtifactTimelineTrackPainterView::TrackClipVisual::Kind::GroupContainer;
    const double trackHeight =
        static_cast<double>(view->trackHeight(clip.trackIndex));
    const double height = isRelationship
  ...
  for (const auto& marker : keyframeMarkers) {
    if (keyframePreview && marker.selected) {
      continue;
    }
  ...
  if (hasDragClip && dragClip.trackIndex >= 0 &&
    ...
    const double x = dragClip.startFrame * ppf - horizontalOffset;
    const double width = std::max(1.0, dragClip.durationFrame * ppf);
    const bool relationship =
        dragClip.kind == ArtifactTimelineTrackPainterView::TrackClipVisual::Kind::Transition ||
        dragClip.kind == ArtifactTimelineTrackPainterView::TrackClipVisual::Kind::GroupContainer;
    const double trackHeight =
        static_cast<double>(view->trackHeight(dragClip.trackIndex));
    ...
    if (x + width >= 0.0 && x <= viewportWidth &&
      ...
      const QColor fill = dragClip.selected ? QColor(39, 104, 169)
                                            : dragClip.fillColor;
      QColor edge(171, 211, 238, 235);
      snapshot.rects.push_back({QRectF(x, top, width, height), fill});
      snapshot.lines.push_back({QPointF(x, top + 0.5),
                                QPointF(x + width, top + 0.5), edge, 1.5f});
      snapshot.lines.push_back({QPointF(x, top + height - 0.5),
                                QPointF(x + width, top + height - 0.5), edge, 1.5f});
      snapshot.lines.push_back({QPointF(x, top), QPointF(x, top + height), edge, 1.5f});
      snapshot.lines.push_back({QPointF(x + width, top),
                                QPointF(x + width, top + height), edge, 1.5f});
      if (!dragClip.title.isEmpty() && width >= 30.0 && height >= 14.0) {
        snapshot.texts.push_back({QPointF(x + 9.0, top + height * 0.69),
                                  ArtifactCore::UniString(dragClip.title),
                                  QColor(242, 246, 250), 10.5f});
      }
    }
  }
  for (const auto& marker : keyframeMarkers) {
    const bool dynamicSelected = keyframePreview && marker.selected;
    if (!dynamicSelected && marker.selected) {
      continue;
    }
    if (!dynamicSelected && std::abs(marker.frame - currentFrame) >= 0.05) {
      continue;
    }
    if (marker.trackIndex < 0 || marker.trackIndex >= view->trackCount()) {
      continue;
  ...
  // Keep the GPU marker close to the adopted low silver Precision Blade.
  snapshot.triangles.push_back({QPointF(playheadX - 7.0, 0.0),
                                QPointF(playheadX + 7.0, 0.0),
                                QPointF(playheadX, 10.0), playheadCapColor});
  snapshot.triangles.push_back({QPointF(playheadX - 1.5, 1.0),
                                QPointF(playheadX + 1.5, 1.0),
                                QPointF(playheadX, 10.0), playheadColor});
...
void ArtifactTimelineWidget::syncPlayheadOverlay()
{
  ...
  if (impl_->rightPanel_) {
    const bool overlayEnabled =
        !impl_->graphEditorVisible_ && !impl_->gpuTimelinePreviewEnabled_;
    setTimelineRightPanelPlayheadOverlayEnabled(impl_->rightPanel_, overlayEnabled);
    if (overlayEnabled) {
...
void ArtifactTimelineWidget::nudgeSelectedKeyframes(const int frameDelta)
{
  if (!impl_ || frameDelta == 0 || !impl_->painterTrackView_) return;
  const auto markers = impl_->painterTrackView_->selectedKeyframeMarkers();
  if (markers.isEmpty()) return;
  QVector<ArtifactTimelineTrackPainterView::KeyframeMarkerVisual> ordered = markers;
  std::sort(ordered.begin(), ordered.end(),
            [frameDelta](const auto& left, const auto& right) {
              return frameDelta > 0 ? left.frame > right.frame
                                     : left.frame < right.frame;
            });
  for (const auto& marker : ordered) {
    ArtifactCore::globalEventBus().publish<TimelineKeyframeMoveRequestedEvent>(
        TimelineKeyframeMoveRequestedEvent{
            marker.layerId.toString(), marker.propertyPath,
            static_cast<qint64>(std::llround(marker.frame)),
            static_cast<qint64>(std::llround(marker.frame)) + frameDelta});
  }
}

void ArtifactTimelineWidget::moveSelectedLayerStartToCurrentFrame()
{
  if (!impl_) return;
  const auto comp = safeCompositionLookup(impl_->compositionId_);
  auto* selection = ArtifactApplicationManager::instance()
      ? ArtifactApplicationManager::instance()->layerSelectionManager() : nullptr;
  if (!comp || !selection) return;
  const qint64 frame = comp->framePosition().framePosition();
  auto layers = selection->selectedLayers();
  if (layers.isEmpty() && selection->currentLayer()) layers.insert(selection->currentLayer());
  auto macro = std::make_unique<MacroUndoCommand>(QStringLiteral("Move Layer Start"));
  for (const auto& layer : layers) {
    if (!layer || layer->isTimingLocked()) continue;
    macro->addChild(std::make_unique<MoveLayerToFrameCommand>(
        impl_->compositionId_, layer->id(), frame,
        captureTimelineLayerStateSnapshots(comp, {layer})));
  }
  if (auto* manager = UndoManager::instance()) manager->push(std::move(macro));
}

void ArtifactTimelineWidget::moveSelectedLayerEndToCurrentFrame()
{
  if (!impl_) return;
  const auto comp = safeCompositionLookup(impl_->compositionId_);
  auto* selection = ArtifactApplicationManager::instance()
  ...
  auto layers = selection->selectedLayers();
  if (layers.isEmpty() && selection->currentLayer()) layers.insert(selection->currentLayer());
  auto macro = std::make_unique<MacroUndoCommand>(QStringLiteral("Move Layer End"));
  for (const auto& layer : layers) {
    if (!layer || layer->isTimingLocked()) continue;
    const qint64 duration = std::max<qint64>(1, layer->outPoint().framePosition() -
                                                layer->inPoint().framePosition());
    macro->addChild(std::make_unique<MoveLayerToFrameCommand>(
        impl_->compositionId_, layer->id(), frame - duration,
        captureTimelineLayerStateSnapshots(comp, {layer})));
  }
  if (auto* manager = UndoManager::instance()) manager->push(std::move(macro));
}

void ArtifactTimelineWidget::trimSelectedLayerInToCurrentFrame()
{
  if (!impl_) return;
  const auto comp = safeCompositionLookup(impl_->compositionId_);
  auto* selection = ArtifactApplicationManager::instance()
  ...
  auto layers = selection->selectedLayers();
  if (layers.isEmpty() && selection->currentLayer()) layers.insert(selection->currentLayer());
  auto macro = std::make_unique<MacroUndoCommand>(QStringLiteral("Trim Layer In"));
  for (const auto& layer : layers) {
    if (!layer || layer->isTimingLocked()) continue;
    const qint64 duration = layer->outPoint().framePosition() - frame;
    if (duration < 1) continue;
    macro->addChild(std::make_unique<TrimLayerToFrameCommand>(
        impl_->compositionId_, layer->id(), frame, duration,
        captureTimelineLayerStateSnapshots(comp, {layer})));
  }
  if (auto* manager = UndoManager::instance()) manager->push(std::move(macro));
...
void ArtifactTimelineWidget::trimSelectedLayerOutToCurrentFrame()
{
  if (!impl_) return;
  const auto comp = safeCompositionLookup(impl_->compositionId_);
  auto* selection = ArtifactApplicationManager::instance()
  ...
  auto layers = selection->selectedLayers();
  if (layers.isEmpty() && selection->currentLayer()) layers.insert(selection->currentLayer());
  auto macro = std::make_unique<MacroUndoCommand>(QStringLiteral("Trim Layer Out"));
  for (const auto& layer : layers) {
    if (!layer || layer->isTimingLocked()) continue;
    const qint64 start = layer->inPoint().framePosition();
    const qint64 duration = frame - start;
    if (duration < 1) continue;
    macro->addChild(std::make_unique<TrimLayerToFrameCommand>(
        impl_->compositionId_, layer->id(), start, duration,
        captureTimelineLayerStateSnapshots(comp, {layer})));
  }
  if (auto* manager = UndoManager::instance()) manager->push(std::move(macro));
}

void ArtifactTimelineWidget::deleteSelectedLayerAnimation()
{
  if (!impl_) return;
  const auto comp = safeCompositionLookup(impl_->compositionId_);
  auto* selection = ArtifactApplicationManager::instance()
  ...
  auto layers = selection->selectedLayers();
  if (layers.isEmpty() && selection->currentLayer()) layers.insert(selection->currentLayer());
  auto macro = std::make_unique<MacroUndoCommand>(QStringLiteral("Delete Animation"));
  for (const auto& layer : layers) {
    if (!layer || layer->isTimingLocked() || collectAnimatablePropertyRefs({layer}).isEmpty())
      continue;
    macro->addChild(std::make_unique<DeleteLayerAnimationCommand>(
        impl_->compositionId_, layer->id(),
        captureTimelineLayerStateSnapshots(comp, {layer})));
  }
  if (auto* manager = UndoManager::instance()) manager->push(std::move(macro));
...
void ArtifactTimelineWidget::distributeSelectedKeyframesEvenly()
{
  if (!impl_ || !impl_->painterTrackView_) return;
  impl_->painterTrackView_->distributeSelectedKeyframeMarkersEvenly();
}
...
bool ArtifactTimelineWidget::hasSelectedKeyframeArea() const
{
  if (!impl_ || !impl_->painterTrackView_) {
    return false;
  }
...
QString ArtifactTimelineWidget::selectedKeyframeAreaSummary() const
{
  if (!impl_ || !impl_->painterTrackView_) {
    return {};
  }
  ...
  return selectedAreaSummaryText(selectedMarkers);
}

bool ArtifactTimelineWidget::applyValueToSelectedKeyframeArea(
    const LayerID& layerId, const QString& propertyPath, const QVariant& value)
{
  if (!impl_ || !impl_->painterTrackView_ || layerId.isNil() ||
      propertyPath.trimmed().isEmpty()) {
    return false;
  }
  ...
  for (auto& snapshot : afterSnapshots) {
    ...
    for (auto& keyframe : snapshot.keyframes) {
      const qint64 frame = keyframe.time.rescaledTo(scale);
      for (const auto& marker : selectedMarkers) {
        if (frame == static_cast<qint64>(std::llround(marker.frame))) {
  ...
  if (auto* mgr = UndoManager::instance()) {
    ...
    undoAccepted = mgr->push(std::make_unique<TimelineKeyframeSnapshotCommand>(
        ...
        [self, composition, afterSnapshots, selectionKeys]() {
          ...
          self->refreshTracks();
          if (self->impl_ && self->impl_->painterTrackView_) {
            self->impl_->painterTrackView_->setSelectedKeyframeKeys(selectionKeys);
          }
          self->updateKeyframeState();
          self->updateSelectionState();
          return applied;
