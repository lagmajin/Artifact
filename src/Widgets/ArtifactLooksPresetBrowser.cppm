...
static const PresetDef kBuiltinPresets[] = {
    {"amber_dusk",     "Amber Dusk",     "Film",      "Looks Classic", 78, {0xc8,0x7a,0x20}},
    {"bleach_bypass",  "Bleach Bypass",  "Film",      "Looks Classic", 85, {0x88,0x99,0x88}},
    {"broadcast_safe", "Broadcast Safe", "TV & Video","Looks Classic", 55, {0x60,0x80,0xa0}},
    {"cold_day",       "Cold Day",       "Photo",     "Cinematic",     70, {0x50,0x70,0xc0}},
    {"cross_process",  "Cross Process",  "Creative",  "Cinematic",     90, {0x80,0x40,0xc0}},
    {"cyberpunk",      "Cyberpunk",      "Creative",  "Cinematic",     92, {0x80,0x00,0xff}},
    {"day_for_night",  "Day For Night",  "Film",      "Cinematic",     88, {0x20,0x30,0x60}},
    {"deep_forest",    "Deep Forest",    "Photo",     "Studio Looks",  80, {0x20,0x50,0x30}},
    {"fuji_velvia",    "Fuji Velvia",    "Film",      "Looks Classic", 80, {0xd0,0x60,0x30}},
    {"golden_hour",    "Golden Hour",    "Photo",     "Studio Looks",  85, {0xd0,0x90,0x20}},
    {"horror_night",   "Horror Night",   "Creative",  "Studio Looks",  95, {0x10,0x10,0x10}},
    {"hyper_real",     "Hyper Real",     "Photo",     "Studio Looks",  90, {0x60,0xa0,0xd0}},
    {"kodak_5219",     "Kodak 5219",     "Film",      "Looks Classic", 90, {0xb0,0x70,0x40}},
    {"log_c_rec709",   "Log C Rec709",   "TV & Video","Cinematic",    100, {0x70,0x80,0x70}},
    {"miami_vice",     "Miami Vice",     "TV & Video","Cinematic",     85, {0x00,0xc0,0xc0}},
    {"muted_indie",    "Muted Indie",    "Film",      "Studio Looks",  70, {0x80,0x80,0xa0}},
    {"news_footage",   "News Footage",   "TV & Video","Looks Classic", 60, {0x70,0x70,0x70}},
    {"noir_bw",        "Noir B&W",       "Creative",  "Looks Classic",100, {0x30,0x30,0x30}},
    {"pastel_dream",   "Pastel Dream",   "Photo",     "カスタム",      65, {0xd0,0xa0,0xb0}},
    {"pro_mist",       "Pro Mist",       "Film",      "カスタム",      75, {0xb0,0xb0,0xc0}},
    {"raw_log_flat",   "Raw Log Flat",   "TV & Video","カスタム",      40, {0x80,0x90,0x80}},
    {"teal_orange",    "Teal & Orange",  "Film",      "カスタム",      75, {0x00,0x80,0x80}},
    {"vintage_70s",    "Vintage 70s",    "Creative",  "Looks Classic", 80, {0xb0,0x80,0x40}},
...
protected:
    void enterEvent(QEnterEvent*) override { hovered_ = true;  updateStyle(); }
    void leaveEvent(QEvent*)      override { hovered_ = false; updateStyle(); }
...
private:
    void updateStyle() {
        if (selected_)
            setStyleSheet("QFrame { border: 2px solid #4a8aff; background:#1a1a1a; border-radius:3px; }");
        else if (hovered_)
            setStyleSheet("QFrame { border: 1px solid #555555; background:#222222; border-radius:3px; }");
        else
            setStyleSheet("QFrame { border: 1px solid #2a2a2a; background:#1a1a1a; border-radius:3px; }");
    }
};
...
public:
    ...
    QWidget* buildPreviewArea(QWidget* parent) {
        ...
        panel->setStyleSheet("background:#1c1c1c; border-bottom:1px solid #2a2a2a;");
        ...
        bvl->setContentsMargins(0,0,0,0); bvl->setSpacing(2);
        ...
        avl->setContentsMargins(0,0,0,0); avl->setSpacing(2);
        ...
        hl->addWidget(buildSliderBlock(panel, "TEMP",      -100, 100,   0,
                                       &tempSlider_,      &tempVal_));
        ...
        QObject::connect(favBtn_, &QPushButton::clicked, [this]() { onFavorite(); });
    ...
    // -----------------------------------------------------------------------
    QWidget* buildSliderBlock(QWidget* parent, const char* label,
                              int min, int max, int def,
                              QSlider** sliderOut, QLabel** valOut)
    {
    ...
    QVector<LooksPreset> filteredPresets() const {
        ...
        for (const auto& p : allPresets_) {
            // Library filter
            if (activeLibrary_ == "お気に入り" && !p.isFavorite)    continue;
            if (activeLibrary_ != "すべて" &&
                activeLibrary_ != "お気に入り" &&
                activeLibrary_ != "最近使用" &&
                p.pack != activeLibrary_)
                continue;
    ...
    void refreshGrid() {
        ...
        for (int i = 0; i < presets.size(); ++i) {
            ...
            QObject::connect(card, &PresetCard::doubleClicked,
                [this](const QString& id) { selectedId_ = id; onApply(); });
        ...
        if (rem != 0) {
            for (int i = rem; i < kCols; ++i) {
                ...
                gridLayout_->addWidget(spacer,
                    presets.size() / kCols, i);
            }
        }
    }
...
LooksPresetBrowserDialog::~LooksPresetBrowserDialog()
{
    delete impl_;
}
...
void LooksPresetBrowserDialog::setPreviewImage(const QPixmap& before)
{
    ...
    if (!before.isNull()) {
        impl_->beforeLabel_->setPixmap(
            before.scaled(impl_->beforeLabel_->size(),
