...
void ArtifactCompositionPlaybackController::setFrameRate(
...
void ArtifactCompositionPlaybackController::setAudioClockProvider(
    const std::function<double()> &provider) {
  impl_->audioClockProvider_ = provider;
}

void ArtifactCompositionPlaybackController::setInOutPoints(
    ArtifactInOutPoints *inOutPoints) {
  impl_->inOutPoints_ = inOutPoints;
  qDebug() << "PlaybackController::setInOutPoints -"
           << (inOutPoints ? "set" : "cleared");
}

ArtifactInOutPoints *
ArtifactCompositionPlaybackController::inOutPoints() const {
...
void ArtifactCompositionPlaybackController::goToNextMarker() {
  ...
  if (nextMarkerPos.has_value()) {
    ...
    qDebug() << "PlaybackController::goToNextMarker -"
             << nextMarkerPos.value().framePosition();
  }
}
...
void ArtifactCompositionPlaybackController::goToPreviousMarker() {
  ...
  auto prevMarkerPos =
      impl_->inOutPoints_->previousMarker(impl_->currentFrame_);
  if (prevMarkerPos.has_value()) {
    goToFrame(prevMarkerPos.value());
    qDebug() << "PlaybackController::goToPreviousMarker -"
             << prevMarkerPos.value().framePosition();
  }
}
...
void ArtifactCompositionPlaybackController::goToNextChapter() {
  ...
  if (nextChapterPos.has_value()) {
    ...
    qDebug() << "PlaybackController::goToNextChapter -"
             << nextChapterPos.value().framePosition();
  }
}
...
void ArtifactCompositionPlaybackController::goToPreviousChapter() {
  ...
  auto prevChapterPos =
      impl_->inOutPoints_->previousChapter(impl_->currentFrame_);
  if (prevChapterPos.has_value()) {
    goToFrame(prevChapterPos.value());
    qDebug() << "PlaybackController::goToPreviousChapter -"
             << prevChapterPos.value().framePosition();
  }
}
...
void ArtifactCompositionPlaybackController::enableOutputMonitoring(
    bool enable) {
  impl_->outputMonitoringEnabled_ = enable;
}

void ArtifactCompositionPlaybackController::setOutputMonitorCallback(
    const std::function<void(bool, bool, const QString &)> &callback) {
  impl_->outputMonitorCallback_ = callback;
}
